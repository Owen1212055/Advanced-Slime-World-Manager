...
public class SWMPlugin extends JavaPlugin implements SlimePlugin {
    ...
    public static SlimeVersionSupport CURRENT_VERSION;
    public static SlimePlatformSupport PLATFORM_SUPPORT;
    ...
    private SlimePlatformSupport getVersionSupport() throws InvalidVersionException {

        int dataVersion = Bukkit.getUnsafe().getDataVersion();
        return switch (dataVersion) {
            case 2730 -> new v1171SlimeNMS();
    ...
    public void onLoad() {
        ...
        try {
            PLATFORM_SUPPORT = getVersionSupport();
            CURRENT_VERSION = PLATFORM_SUPPORT.getVersion();
        ...
        try {
            ...
            //CURRENT_VERSION.setDefaultWorlds(defaultWorld, netherWorld, endWorld);
    ...
    public void onDisable() {
        Bukkit.getWorlds().stream()
                .map(world -> PLATFORM_SUPPORT.getSlimeWorld(world))
                .filter(Objects::nonNull)
                .filter((slimeWorld -> !slimeWorld.isReadOnly()))
                .map(w -> (LegacyCraftSlimeWorld) w)
                .forEach(w -> {
                    try {
    ...
    public SlimeWorld loadWorld(SlimeLoader loader, String worldName, boolean readOnly, SlimePropertyMap propertyMap) throws UnknownWorldException, IOException,
        ...
        try {
            world = CURRENT_VERSION.readWorld(loader, worldName, serializedWorld, propertyMap, readOnly);
        } catch (Exception ex) {
    ...
    public SlimeWorld createEmptyWorld(SlimeLoader loader, String worldName, boolean readOnly, SlimePropertyMap propertyMap) throws WorldAlreadyExistsException, IOException {
        ...
        long start = System.currentTimeMillis();
        SlimeWorld world = CURRENT_VERSION.createEmptyWorld(loader, worldName, readOnly, propertyMap);
        loader.saveWorld(worldName, CURRENT_VERSION.serializeWorld(world), !readOnly);
    ...
    public void generateWorld(SlimeWorld slimeWorld) {
        ...
        PLATFORM_SUPPORT.registerWorld(slimeWorld);
    ...
    public void migrateWorld(String worldName, SlimeLoader currentLoader, SlimeLoader newLoader) throws IOException,
        ...
        if (bukkitWorld != null) {
            ...
            LegacyCraftSlimeWorld slimeWorld = (LegacyCraftSlimeWorld) SWMPlugin.PLATFORM_SUPPORT.getSlimeWorld(bukkitWorld);
...
//
//        if (bukkitWorld != null && CURRENT_VERSION.getSlimeWorld(bukkitWorld) == null) {
//            throw new WorldLoadedException(worldDir.getName());
//        }
//
//        LegacyCraftSlimeWorld world = WorldImporter.readFromDirectory(worldDir);
//
