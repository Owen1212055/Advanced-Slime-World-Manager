...
public class WorldImporter {
    ...
    public static LegacyCraftSlimeWorld readFromDirectory(File worldDir) throws InvalidWorldException, IOException {
        ...
        return new LegacyCraftSlimeWorld(null, worldDir.getName(), chunks, new CompoundTag("", extraData),
    ...
    private static SlimeChunk readChunk(CompoundTag compound, byte worldVersion) {
        ...
        for (CompoundTag sectionTag : sectionsTag.getValue()) {
            ...
            sectionArray[index] = new LegacyCraftSlimeChunkSection(blocks, dataArray, paletteTag, blockStatesArray, blockLightArray, skyLightArray);
        }

        for (SlimeChunkSection section : sectionArray) {
            if (section != null) { // Chunk isn't empty
                return new LegacyCraftSlimeChunk(null, chunkX, chunkZ, sectionArray, heightMapsCompound, biomes, tileEntities, entities);
            }
        }
