package com.grinderwolf.swm.core.version.legacy.upgrade.v1_16;
...
public class v1_16WorldUpgrade implements Upgrade {
    ...
    @Override
    public void upgrade(LegacyCraftSlimeWorld world) {
        for (SlimeChunk chunk : new ArrayList<>(world.getChunks().values())) {
            ...
            for (int sectionIndex = 0; sectionIndex < chunk.getSections().length; sectionIndex++) {
                ...
                if (section != null) {
                    ...
                    if (!isPowerOfTwo(bitsPerBlock)) {
                        section = new LegacyCraftSlimeChunkSection(null, null, section.getPalette(),
            ...
            world.updateChunk(new LegacyCraftSlimeChunk(chunk.getWorldName(), chunk.getX(), chunk.getZ(),
                    chunk.getSections(), chunk.getHeightMaps(), newBiomes,
                    chunk.getTileEntities(), chunk.getEntities(), ((LegacyCraftSlimeChunk) chunk).getUpgradeData()));
        }
    }
